...
1. **Why didn't I apply the same change to the `test` set?:** Because `test` doesn't have a SalePrice variable to change - it is our unknown Y
1. **If I transformed the outcome variable now, what will I have to make sure to do later on?** You will have to make sure to convert it back from logit space to positive space
...
```{r}
model_formula <- "logSalePrice ~ MSSubClass + LotFrontage + LotArea + OverallQual + OverallCond + YearBuilt + YearRemodAdd + MasVnrArea + BsmtFinSF1 +BsmtFinSF2 + BsmtUnfSF + TotalBsmtSF + FirstFlrSF + SecondFlrSF + LowQualFinSF + GrLivArea + BsmtFullBath + BsmtHalfBath + FullBath + HalfBath + BedroomAbvGr + KitchenAbvGr + TotRmsAbvGrd + Fireplaces + GarageYrBlt + GarageCars + GarageArea + WoodDeckSF + OpenPorchSF + EnclosedPorch + ThirdSsnPorch + ScreenPorch + PoolArea +MiscVal + MoSold + YrSold" %>% 
  as.formula()
```
...
![](images/screenshot_rf.png){ width=100% }
